...
class AttendanceController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($s  = $request->search)     $query->whereHas('employee', fn($q) => $q->where('first_name','like',"%$s%")->orWhere('last_name','like',"%$s%"));
        if ($d  = $request->date)       $query->whereDate('date', $d);
    ...
    public function timeIn(Request $request)
    {
        ...
        if ($existing) return back()->with('error', 'Already timed in today.');
    ...
    public function timeOut(Request $request)
    {
        ...
        if (!$record) return back()->with('error', 'No active time-in found.');

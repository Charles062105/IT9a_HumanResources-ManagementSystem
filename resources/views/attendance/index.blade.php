<x-app-layout title="Attendance" crumb="HR · Attendance tracking">
            ...
            <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
                ...
                @if(!$todayRecord)
                    <form method="POST" action="{{ route('attendance.time-in') }}" data-clock-form
                        style="margin:0;display:flex;align-items:center;gap:8px;flex-wrap:wrap" id="clock-in-form">
                        @csrf
    ...
    @if(auth()->user()->isEmployee())
        <div class="live-clock-card">
            <div class="live-clock-inner">
                <div class="live-clock-left">
                    <div id="live-clock" class="live-clock-time">--:--:--</div>
                    <div id="live-date" class="live-clock-date">Today</div>
                </div>

                @if($todayRecord)
                    <div class="live-clock-right">
                        <div class="live-clock-times">
                            <div class="live-clock-time-block">
                                <div class="live-clock-time-label">Clocked In</div>
                                <div class="live-clock-time-value live-clock-in">
                                    {{ $todayRecord->time_in?->format('h:i A') ?? '—' }}</div>
                            </div>
                            <div class="live-clock-divider"></div>
                            <div class="live-clock-time-block">
                                <div class="live-clock-time-label">Clocked Out</div>
                                <div class="live-clock-time-value live-clock-out">
                                    {{ $todayRecord->time_out?->format('h:i A') ?? '—' }}</div>
                            </div>
                        </div>
                        @php
                            $status = match (true) {
                        ...
                        @endphp
                        <div class="live-clock-status status-{{ $status }}">
                            @if($status === 'complete')
                                ✓ Day Complete
                ...
                @else
                    <div class="live-clock-right live-clock-empty">
                        Not yet clocked in
                    </div>
                @endif
            </div>

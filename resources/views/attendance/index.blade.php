<x-app-layout title="Attendance" crumb="HR · Attendance tracking">
            ...
            <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
                @if(auth()->user()->employee->shift)
                    <div class="shift-chip">
                        ...
                        {{ auth()->user()->employee->shift->name }}:
                        {{ auth()->user()->employee->shift->start_time->format('H:i') }}–{{ auth()->user()->employee->shift->end_time->format('H:i') }}
                    </div>
                @endif

                @if(!$todayRecord)
                    <form method="POST" action="{{ route('attendance.time-in') }}" data-clock-form
                        style="margin:0;display:none;align-items:center;gap:8px;flex-wrap:wrap" id="clock-in-form">
                        @csrf
                        <input type="time" name="time" class="form-input form-input-time" placeholder="HH:MM"
                            aria-label="Clock time input" required id="clock-in-time">
                    </form>
                ...
                @elseif($todayRecord->time_in && !$todayRecord->time_out)
                    <form method="POST" action="{{ route('attendance.time-out') }}" data-clock-form
                        style="margin:0;display:none;align-items:center;gap:8px;flex-wrap:wrap"
                        id="clock-out-form">
                        @csrf
                        <input type="time" name="time" class="form-input form-input-time" placeholder="HH:MM"
                            aria-label="Clock time input" required id="clock-out-time">
                    </form>
                    <button type="button" class="btn-danger confirm-clock-out" data-action="time-out"
                ...
                @else
                    <div class="complete-badge">
                        ...
                        @endphp
                        Done — {{ $todayRecord->time_out->format('h:i A') }} · {{ $whole }}h {{ sprintf('%02d', $mins) }}m
                        worked
                    </div>
                @endif
            </div>
    ...
    @if(auth()->user()->isEmployee())
        <div class="section-card clock-section"
            style="background:linear-gradient(135deg,#0f1e38 0%,#1a2744 100%);color:white;padding:32px;margin-bottom:24px">
            <div style="display:flex;align-items:center;gap:32px;flex-wrap:wrap;flex-direction:row">
                <div style="flex:1;text-align:center;min-width:200px">
                    <div id="live-clock" class="clock-display"
                        style="font-family:'DM Mono',monospace;font-size:3.5rem;font-weight:700;color:white;line-height:1.1">
                        --:--:--</div>
                    <div id="live-date" class="clock-date"
                        style="text-align:center;font-size:14px;color:rgba(255,255,255,0.6);margin-top:8px;font-weight:500">
                        Today</div>
                </div>
                ...
                @if($todayRecord)
                            <div style="flex:1">
                                <div class="clock-info-box"
                                    style="display:flex;align-items:center;gap:16px;margin-bottom:16px;background:rgba(255,255,255,0.08);padding:16px;border-radius:8px">
                                    <div style="flex:1">
                                        <div class="clock-label"
                                            style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;color:rgba(255,255,255,0.5);margin-bottom:4px">
                                            Clocked In</div>
                                        <div style="font-size:24px;font-weight:700;font-family:'DM Mono',monospace;color:#4ade80">
                                            {{ $todayRecord->time_in?->format('h:i A') ?? '—' }}
                                        </div>
                                    </div>
                                    <div class="divider-vertical" style="width:1px;height:40px;background:rgba(255,255,255,0.15)"></div>
                                    <div style="flex:1">
                                        <div class="clock-label"
                                            style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;color:rgba(255,255,255,0.5);margin-bottom:4px">
                                            Clocked Out</div>
                                        <div style="font-size:24px;font-weight:700;font-family:'DM Mono',monospace;color:#f87171">
                                            {{ $todayRecord->time_out?->format('h:i A') ?? '—' }}
                                        </div>
                                    </div>
                                </div>
                      @php
                    ...
                    @endphp
                                <div class="clock-status-badge" style="display:inline-flex;align-items:center;gap:8px;padding:10px 16px;border-radius:6px;font-size:13px;font-weight:600;width:100%;justify-content:center;
                                    @if($status === 'complete')
    ...
    <div class="section-card">
        <form method="GET" action="{{ route('attendance.index') }}" class="filter-bar">
            ...
            <input class="finput" type="date" name="date_from" value="{{ request('date_from') }}" style="width:130px"
                title="Date from">
            <span class="fb-label" style="font-size:9px">To</span>
            <input class="finput" type="date" name="date_to" value="{{ request('date_to') }}" style="width:130px"
                title="Date to">
            ...
            <table>
                ...
                <tbody>
                    @forelse($records as $r)
                        <tr
                            class="{{ auth()->user()->isAdmin() && auth()->user()->employee?->id === $r->employee_id ? 'row-own' : '' }}">
                            <td>
                                <div style="display:flex;align-items:center;gap:8px">
                                    <div class="av" style="background:#DBEAFE;color:#1E40AF">{{ $r->employee?->initials }}
                                    </div>
                            ...
                            <td class="td-muted"
                                style="max-width:140px;white-space:normal;word-break:break-word;font-size:11px">
                                @if($r->employee?->shift)
                                    <span style="font-weight:500">{{ $r->employee->shift->name }}</span>
                                    <span
                                        style="color:var(--text3)">({{ $r->employee->shift->start_time->format('H:i') }}–{{ $r->employee->shift->end_time->format('H:i') }})</span>
                                @else
                                    —
                                @endif
                            </td>
                            <td class="td-muted" style="white-space:nowrap">{{ $r->date->format('M j, Y') }}</td>
                            <td class="{{ $r->status === 'late' ? 'td-late' : 'td-muted' }}"
                            ...
                            <td class="td-muted" style="font-family:'DM Mono',monospace;font-size:11px">
                                {{ $r->time_out?->format('h:i A') ?? '—' }}
                            </td>
                            ...
                            <td>
                                ...
                                @endphp
                                <span class="sp {{ $sc }}"><span
                                        class="d"></span>{{ ucfirst(str_replace('_', ' ', $r->status)) }}</span>
                            </td>
                            @if(auth()->user()->isAdmin())
                                <td>
                                    <div class="table-actions">
                                        <a href="{{ route('attendance.edit', $r) }}"
                                            class="notif-action-btn notif-action-btn-read"
                                            style="padding:3px 9px;font-size:10px;border-radius:5px;text-decoration:none">Edit</a>
                                    </div>
                                </td>
                            @endif
                        </tr>

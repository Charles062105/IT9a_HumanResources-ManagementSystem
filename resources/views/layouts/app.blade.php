...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600&family=DM+Mono:wght@400;500&family=Syne:wght@600;700&display=swap"
        rel="stylesheet">
...
<body>
    ...
    <div class="layout">
        ...
        <button class="sidebar-toggle" id="sidebarToggle" aria-label="Toggle sidebar menu" aria-expanded="false">
            <span></span>
            <span></span>
            <span></span>
        </button>
            ...
            <div class="sb-top">
                ...
                @if(auth()->user()->isAdmin())
                    <a href="{{ route('employees.index') }}"
                        class="nl {{ request()->routeIs('employees.*') ? 'active' : '' }}">
                        <svg class="ni" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <a href="{{ route('attendance.index') }}"
                    class="nl {{ request()->routeIs('attendance.*') ? 'active' : '' }}">
                    <svg class="ni" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <a href="{{ route('timesheets.index') }}"
                    class="nl {{ request()->routeIs('timesheets.*') ? 'active' : '' }}">
                    <svg class="ni" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <a href="{{ route('leaves.index') }}" class="nl {{ request()->routeIs('leaves.*') ? 'active' : '' }}">
                    ...
                    Leaves
                    @if($sidebarCounts['pendingLeaves']) <span class="nb">{{ $sidebarCounts['pendingLeaves'] }}</span>
                    @endif
                </a>
                <a href="{{ route('violations.index') }}"
                    class="nl {{ request()->routeIs('violations.*') ? 'active' : '' }}">
                    <svg class="ni" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" />
                    </svg>
                    Violations
                    @if($sidebarCounts['openViolations']) <span class="nb">{{ $sidebarCounts['openViolations'] }}</span>
                    @endif
                </a>
                <a href="{{ route('performance.index') }}"
                    class="nl {{ request()->routeIs('performance.*') ? 'active' : '' }}">
                    <svg class="ni" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                @if(auth()->user()->isAdmin())
                    ...
                    <a href="{{ route('notifications.index') }}"
                        class="nl {{ request()->routeIs('notifications.*') ? 'active' : '' }}">
                        <svg class="ni" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9" />
                            <path d="M13.73 21a2 2 0 0 1-3.46 0" />
                        </svg>
                        Notifications
                        @if($sidebarCounts['unreadNotifications']) <span
                        class="nb">{{ $sidebarCounts['unreadNotifications'] }}</span> @endif
                    </a>
                    <a href="{{ route('requests.index') }}"
                        class="nl {{ request()->routeIs('requests.*') ? 'active' : '' }}">
                        <svg class="ni" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <circle cx="12" cy="12" r="3" />
                            <path d="M19.07 4.93a10 10 0 0 1 0 14.14M4.93 4.93a10 10 0 0 0 0 14.14" />
                        </svg>
                        Requests
                        @if($sidebarCounts['pendingRequests']) <span
                        class="nb">{{ $sidebarCounts['pendingRequests'] }}</span> @endif
                    </a>
                @endif
            </div>
        ...
        <div class="main">
            ...
            <header class="topbar">
                ...
                <div class="tb-right">
                    ...
                    <a href="{{ route('notifications.index') }}" class="tib notif-bell"
                        style="position:relative;text-decoration:none;display:flex;align-items:center;justify-content:center">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="15"
                            height="15" style="color:#475569">
                            <path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9" />
                            <path d="M13.73 21a2 2 0 0 1-3.46 0" />
                        </svg>
                        @if($sidebarCounts['unreadNotifications']) <span
                            class="notif-count">{{ $sidebarCounts['unreadNotifications'] > 99 ? '99+' : $sidebarCounts['unreadNotifications'] }}</span>
                        @endif
                    </a>
                    <a href="{{ route('profile.edit') }}" class="u-av tib-av"
                        title="Profile">{{ strtoupper(substr(auth()->user()->name, 0, 2)) }}</a>
                </div>
            </header>
    ...
    <div id="logoutConfirmationModal" class="logout-confirmation-modal">
        <div class="lc-content">
            ...
            <p class="lc-message">Are you sure you want to log out? You will need to log in again to access the system.
            </p>
    ...
    <script>
        ...
        // Live clock display for attendance
